...
void PCoA::Project(const Matrix& distMatrix)
{
	...
	for(int r = 0; r < m_projectedData.size(); ++r)
	{
		for(int c = 0; c < m_projectedData.size(); ++c)
		{			
	...
	QVector<double> colMeans(m_projectedData.size(), 0);
	QVector<double> rowMeans(m_projectedData.size(), 0);
	for(int r = 0; r < m_projectedData.size(); ++r)
	{
		for(int c = 0; c < m_projectedData.size(); ++c)
		{			
	...
	for(int i = 0; i < m_projectedData.size(); ++i)
	{
	...
	for(int i = 0; i < m_projectedData.size(); ++i)
	{
		if(D[i] < 0)
		{
			Log::Inst().Write("There are negative eigenvectors. These are currently not being corrected.");
			D[i] = -D[i];
			for(int j = 0; j < m_projectedData.size(); ++j)
	...
	qSort(m_eigenValues.begin(), m_eigenValues.end(), qGreater<double>());

	QList<uint> indexOrder;
	for(int i = 0; i < m_projectedData.size(); ++i)
	{
		bool bBreak = false;
		for(int j = 0; j < m_projectedData.size() && !bBreak; ++j)
		{
	...
	for(int i = 0; i < m_projectedData.size(); ++i)
	{
		m_variance.append(m_eigenValues.at(i) / m_sumEigenvalues);

		if(i == 0)
			m_sumVariance.append(m_variance.at(0));
		else
			m_sumVariance.append(m_sumVariance.at(i-1) + m_variance.at(i));
	}
...
void PCoA::WriteToFile(const QString& filename)
{
	QFile file(filename);
	if(!file.open(QIODevice::WriteOnly | QIODevice::Text))
	{
		Log::Inst().Warning("Unable to create file: " + file.fileName());
		return;
	}
	...
	out << "Projected data (each column is a point): " << endl;
	for(int i = 0; i < m_projectedData.at(0).size(); ++i)
	{
		for(int ptIndex = 0; ptIndex < m_projectedData.size(); ++ptIndex)
		{
			out << m_projectedData.at(ptIndex).at(i) << '\t';
		}
		out << endl;
	}

	// write captured variance
	out << endl << "Captured variance for each dimension: " << endl;
	for(int i = 0; i < m_variance.size(); ++i)
		out << m_variance.at(i) << endl;
